	...
	<div class="card-body">
		<div class="form-row">
			...
			<div class="col-sm-12 col-md-12 col-lg-3">
				...
				<select class="custom-select" required name="graduacao_id">
		      <option value="">Escolha uma opção...</option>
					@foreach ($graduacao as $key => $g)
						<option value="{{ $g->id }}" {{ isset($registro->graduacao_id) && $registro->graduacao_id == $g->id ? 'selected' : '' }}>{{ $g->nomeGraduacao }}</option>
					@endforeach
		    </select>
			</div>

			<div class="col-sm-12 col-md-12 col-lg-4">
				<label>Peso (Ex. 100.00)</label>
				<input type="text" name="peso" class="form-control" value="{{ isset($registro->peso) ? $registro->peso : '' }}" required>
			</div>

			<div class="col-sm-12 col-md-12 col-lg-4">
				<label>Academia</label>
				<select class="custom-select" required name="academia_id">
		      <option value="">Escolha uma opção...</option>
					@foreach ($academia as $key => $a)
						<option value="{{ $a->id }}" {{ isset($registro->academia_id) && $registro->academia_id == $a->id ? 'selected' : '' }}>{{ $a->nome }}</option>
					@endforeach
		    </select>
			</div>
